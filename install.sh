...
[[ -e /bin/ejecutar/msg ]] && source /bin/ejecutar/msg > /dev/null || source <(curl -sSL https://gitlab.com/patomods/patobase/-/raw/main/sources/msg) > /dev/null
numero='^[0-9]+$'
hora=$(printf '%(%H:%M:%S)T') 
fecha=$(printf '%(%D)T')
...
install(){
	...
	#restart_v2r
	echo "source <(curl -sSL https://gitlab.com/patomods/patobase/-/raw/main/sources/xray_manager.sh)" > /bin/v2r.sh
	chmod +x /bin/r.sh
}
...
enon(){
echo "source <(curl -sSL  https://gitlab.com/patomods/patobase/-/raw/main/sources/xray_manager.sh)" > /bin/xr.sh
chmod +x /bin/xr.sh
		clear
		msg -bar3
