...
function print_success() {
    if [[ 0 -eq $? ]]; then
		echo -e "${green} =============================== ${FONT}"
        echo -e "${Green} # $1 INSTALACION EXITOSO"
		echo -e "${green} =============================== ${FONT}"
        sleep 2
    fi
}
...
# Buat direktori xray
print_install "Creando directorios xray"
    mkdir -p /etc/xray
...
echo -e "     \e[1;32m2)\e[0m Subdominio aleatorio"
echo -e "   ------------------------------------"
read -p "   Selecciona una opcion 1 o 2 : " host
echo ""
if [[ $host == "1" ]]; then
...
print_install "Random Subdominio/Domain is Used"
...
function password_default() {
    ...
    <code>Usuario :</code> <code>$author</code>
    <code>────────────────────</code>
...
function pasang_ssl() {
clear
print_install "Certificacion de dominio automatico"
...
clear
    print_install "Core Xray 1.8.1 Ultima Version"
    # install xray
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    print_success "Core Xray 1.8.1 Nueva Version"
    
    # Settings UP Nginix Server
    clear
    curl -s ipinfo.io/city >>/etc/xray/city
    curl -s ipinfo.io/org | cut -d " " -f 2-10 >>/etc/xray/isp
    print_install "Instalación configuración de paquetes"
...
clear
print_install "Reiniciando servicios"
